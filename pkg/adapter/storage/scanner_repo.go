...
func (r *scannerRepo) createScheduleWithNextRunTime(db *gorm.DB, scannerID int64, schedule *scannerDomain.Schedule) error {
	...
	nextRunTime := scheduler.CalculateNextRunTime(domainSchedule, time.Now())
...
func (r *scannerRepo) DeleteBatch(ctx context.Context, scannerIDs []int64) (int, error) {
	if len(scannerIDs) == 0 {
		return 0, nil
	}

	log.Printf("Repository: Batch deleting %d scanners", len(scannerIDs))
	...
	result := db.Table("scanners").
		...
		Where("deleted_at IS NULL"). // Only delete non-deleted scanners
		Update("deleted_at", now)

	if result.Error != nil {
		log.Printf("Repository: Error batch deleting scanners: %v", result.Error)
		return 0, result.Error
	}

	log.Printf("Repository: Successfully deleted %d scanners", result.RowsAffected)
...
func (r *scannerRepo) List(ctx context.Context, filter domain.ScannerFilter, pagination domain.Pagination) ([]domain.ScannerDomain, int, error) {
	...
	for _, s := range scanners {
		...
		// We use error ignoring here to get partial data even if some relationships fail
		switch scanner.ScanType {
		case domain.ScannerTypeNmap:
...
func (r *scannerRepo) BatchUpdateEnabled(ctx context.Context, scannerIDs []int64, Status bool) (int, error) {
	log.Printf("Repository: Batch updating %d scanners to status=%v", len(scannerIDs), Status)
...
func (r *scannerRepo) Update(ctx context.Context, scanner domain.ScannerDomain) error {
	...
	if scanner.Schedule != nil {
		...
		if count > 0 {
			...
			nextRunTime := scheduler.CalculateNextRunTime(schedule, time.Now())
...
func (r *scannerRepo) GetByID(ctx context.Context, scannerID int64) (*domain.ScannerDomain, error) {
	...
	domainScanner := &domain.ScannerDomain{
		ID:        scanner.ID,
		Name:      scanner.Name,
		ScanType:  scanner.ScanType, // Using string directly
		Status:    scanner.Status,
		CreatedAt: scanner.CreatedAt,
	}
