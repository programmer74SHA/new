...
func (s *schedulerService) ExecuteScheduledScan(ctx context.Context, scheduledScan domain.ScheduledScan) error {
	...
	nextRunTime := CalculateNextRunTime(scheduledScan.Schedule, currentTime)
	...
	go func(scanner scannerDomain.ScannerDomain, jobID int64) {
		...
		log.Printf("Scheduler Service: Executing scan for scanner type: '%s'", scanner.ScanType)
		...
		var scanErr error
		switch scanType {
		case strings.ToUpper(scannerDomain.ScannerTypeNmap): // Match case-insensitively
			log.Printf("Scheduler Service: Executing NMAP scan for job ID: %d", jobID)
			scanErr = s.nmapScanner.ExecuteNmapScan(bgCtx, scanner, jobID)
		case strings.ToUpper(scannerDomain.ScannerTypeVCenter): // Match case-insensitively
			log.Printf("Scheduler Service: Executing VCenter scan for job ID: %d", jobID)
		...
		case strings.ToUpper(scannerDomain.ScannerTypeDomain): // Match case-insensitively
			log.Printf("Scheduler Service: Executing Domain scan for job ID: %d", jobID)
		...
		if scanErr != nil {
			if errors.Is(scanErr, context.Canceled) {
				log.Printf("Scheduler Service: Scan was cancelled for job ID: %d", jobID)
				err := s.CompleteScanJob(bgCtx, jobID, domain.ScheduleStatusCancelled)
				if err != nil {
					log.Printf("Scheduler Service: Failed to update job status to cancelled: %v", err)
				}
			} else {
...
func (s *schedulerService) CancelScanJob(ctx context.Context, jobID int64) error {
	...
	// Log the job type to help with debugging
	log.Printf("Scheduler Service: Job type: '%s'", job.Type)

	// Normalize job type for comparison
	jobType := strings.TrimSpace(strings.ToUpper(job.Type))
	log.Printf("Scheduler Service: Normalized job type: '%s'", jobType)

	// Determine scanner type and use appropriate cancel method
	var cancelled bool
	switch jobType {
	case strings.ToUpper(scannerDomain.ScannerTypeNmap):
		log.Printf("Scheduler Service: Cancelling NMAP scan job")
		cancelled = s.nmapScanner.CancelScan(jobID)
	case strings.ToUpper(scannerDomain.ScannerTypeVCenter):
		log.Printf("Scheduler Service: Cancelling VCenter scan job")
		if s.vcenterScanner == nil {
	...
	case strings.ToUpper(scannerDomain.ScannerTypeDomain):
		log.Printf("Scheduler Service: Cancelling Domain scan job")
		if s.domainScanner == nil {
