...
func CalculateNextRunTime(schedule scannerDomain.Schedule, from time.Time) time.Time {
	...
	case scannerDomain.ScheduleTypeRecurring:
		// For recurring schedules, use the existing logic
		nextRun := calculateRecurringTime(schedule, from)
		log.Printf("Recurring schedule: calculated time %v", nextRun)
		return nextRun

	default:
		// Default to recurring behavior for backwards compatibility
		log.Printf("Unknown schedule type %s, defaulting to recurring", schedule.ScheduleType)
		return calculateRecurringTime(schedule, from)
	}
}
...
// calculateRecurringTime calculates the next run time for recurring schedules (existing logic)
func calculateRecurringTime(schedule scannerDomain.Schedule, from time.Time) time.Time {
