...
func (s *AssetService) DeleteAsset(ctx context.Context, assetUUID uuid.UUID) (*pb.DeleteAssetResponse, error) {
	if assetUUID == uuid.Nil {
		return nil, ErrInvalidAssetUUID
	}

	err := s.service.DeleteAsset(ctx, assetUUID)
	if err != nil {
		return nil, err
	}
...
func (s *AssetService) DeleteAssets(ctx context.Context, assetUUIDs []uuid.UUID) (*pb.DeleteAssetsResponse, error) {
	if len(assetUUIDs) == 0 {
	...
	// Convert []uuid.UUID to []domain.AssetUUID
	domainUUIDs := make([]domain.AssetUUID, 0, len(assetUUIDs))
	for _, assetUUID := range assetUUIDs {
		if assetUUID == uuid.Nil {
			return nil, ErrInvalidAssetUUID
		}
		domainUUIDs = append(domainUUIDs, assetUUID)
	}

	err := s.service.DeleteAssets(ctx, domainUUIDs)
	if err != nil {
		return nil, err
	}
